    ...
    def get_optimal_node(
        ...
        if strategy == "active_inference":
            ...
            candidates = [
                node
                for node in utility.keys()
                if np.abs(utility[node].mean() - maximum_U)
                < 1e-5
            ]
            return random.choice(candidates)
...
class MABSimulator:
    def __init__(
        ...
        true_thetas=None,  # NEW: Allow passing in pre-generated true thetas
    ):
        ...
        self.iteration_data = (
            []
        )  # New: store iteration-level data
        self.true_thetas = (
            true_thetas  # NEW: Store the true thetas
        )

    def add_participant(self, participant_id):
        # Randomly assign treatment condition z
        self.data["participants"][participant_id] = {
            "z": np.random.binomial(1, 0.5)
        }

    def generate_outcome(self, node_id, z):
        """Generate outcome y based on true theta and treatment condition z"""
        theta = self.true_thetas[node_id][z]
    ...
    def get_current_best_treatment(self):
        ...
        utilities = calculate_current_utilities(
            self.data, self.nodes
        )
        if not utilities:
            return None
        ...
        return max(
            utilities.keys(),
            key=lambda node: utilities[node],
        )
    ...
    def get_true_best_treatment(self):
        ...
        for node_id in self.nodes:
            phi = np.array(
                [
                    self.true_thetas[node_id][0],
                    self.true_thetas[node_id][1],
                ]
            )
            true_utilities[node_id] = phi[0] + phi[1]

        return max(
            true_utilities.keys(),
            key=lambda node: true_utilities[node],
        )
    ...
    def process_participant(self, participant_id):
        participant_z = self.data["participants"][
            participant_id
        ]["z"]
        ...
        n_treatments = (
            ...
            else self.n_trials_per_participant  # Only test 5 treatments per participant
        )
        ...
        for i in range(n_treatments):
            ...
            y = self.generate_outcome(
                next_node, participant_z
            )

            # Store the trial data
            self.data["nodes"][next_node][
                self.trial_counter
            ] = {
                "y": int(y),
                "z": int(participant_z),
            ...
            # Calculate reward for first 5 trials only
            ...
            true_best_utility = self.get_true_utility(
                true_best
            )
            ...
            self.iteration_data.append(
                {
                    ...
                    "treatment_condition_z": participant_z,
                    "outcome_y": int(y),
                    ...
                    "true_utility_treatment": self.get_true_utility(
                        next_node
                    ),
    ...
    def simulate(self, run_id=0):
        ...
        # NEW: Only generate true_thetas if not already provided
        ...
        self.total_reward = 0
        self.trial_counter = 0
        self.iteration_data = []  # Reset iteration data
...
def run_single_simulation(simulation_params):
    ...
    Parameters:
    - simulation_params: Tuple containing (strategy, gamma, nodes_ids, run_id, true_thetas)

    Returns:
    - List of iteration data dictionaries
    """
    strategy, gamma, nodes_ids, run_id, true_thetas = (
        simulation_params
    )
    ...
    # Run simulation with the provided true_thetas
    simulator = MABSimulator(
        strategy=strategy,
        nodes=nodes_ids,
        gamma=gamma,
        true_thetas=true_thetas,  # NEW: Pass the true thetas
    )
...
def evaluate_strategies(
    ...
    # NEW: Generate the true MABs once for all strategies
    print(f"Generating {n_simulations} random MABs...")
    mab_instances = []
    for run_id in range(n_simulations):
        # Set seed for reproducible MAB generation
        np.random.seed(
            run_id + 1000
        )  # Offset to avoid conflicts with simulation seeds
        true_thetas = {
            node_id: {
                0: np.random.beta(2, 2),  # theta for z=0
                1: np.random.beta(2, 2),  # theta for z=1
            }
            for node_id in nodes_ids
        }
    ...
    for strategy in strategies:
        ...
        # NEW: Prepare parameters with the same MAB instances for each strategy
        simulation_params = [
            (
                strategy_name,
                gamma,
                ...
                mab_instances[
                    run_id
                ],  # Same MAB instance for each strategy
            )
            for run_id in range(n_simulations)
        ]
...
def simulate(strategies, output_iterations):
    ...
    print(
        f"Iteration-level data saved to {output_iterations}"
    )
...
def plot_combined_regret(df):
    ...
    plt.savefig(
        "output/systematic_active_inference.pdf",
        bbox_inches="tight",
    )
...
if __name__ == "__main__":
    ...
    output_iterations = "output/mab_iterations_400.csv"
